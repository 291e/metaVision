...
import { AllProductQuery, AllProductQueryVariables, Product } from "@/app/gql/graphql";
import Image from "next/image";

const Slide: React.FC = () => {
	// Apollo Client의 useQuery 훅을 사용하여 데이터 패칭
	const { data, loading, error } = useQuery<AllProductQuery, AllProductQueryVariables>(ALL_PRODUCT_QUERY, {
	...
	// 데이터가 없거나 비어있는 경우 처리
	if (!data || !data.allProduct || data.allProduct.length === 0) return <div>상품이 없습니다.</div>;
	...
	return (
		<Swiper
			...
			{latestProducts.map((product) => (
				<SwiperSlide className="py-8 px-4" key={product.id}>
...
const HoverableProduct: React.FC<{ product: Product }> = ({ product }) => {
	...
	return (
		<div
			...
			{isHovered && product.result_usda ? (
				<iframe
					src={product.result_usda.startsWith("http") ? product.result_usda : `http://${product.result_usda}`}
					width={250}
					height={250}
				...
				<Link href={`/meta360/${product.id}`} className="relative size-60 rounded-md mx-auto overflow-hidden">
					{product.original_photo && product.original_photo.length > 0 && product.original_photo[0] ? (
						<Image
